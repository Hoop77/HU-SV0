...
def plot_all(sig):
    ...
    ax.axhline(y=sig.median(), label="Median", color=(0.6, 0.6, 0.6, 1.0))
    ...
    ax.axhline(y=sig.max_value(), label="Maximum", color=(0.4, 0.0, 0.4, 1.0))
    ...
    ax.axhline(y=sig.central_moment(k=3, normed=True), label="3. normierte Schiefe", color=(0.6, 0.6, 0.6, 1.0))
    ...
    ax.axhline(y=sig.entropy(), label="Entropy", color=(0.4, 0.0, 0.4, 1.0))
    legend = ax.legend(loc='upper right', shadow=True)
    # Set the fontsize
    for label in legend.get_texts():
        label.set_fontsize('small')
